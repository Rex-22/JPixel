...
public class Level implements IGameObject {
    ...
    private void loadLevel() {
        ...
        for (int y = 0; y < level_data.getHeight(); y++) {
            for (int x = 0; x < level_data.getWidth(); x++) {
                ...
                if (col == 0xFFFFD800) {
                    Entity coin = new Entity(new Vec2(x * Tile.SIZE, y * Tile.SIZE), "coin", Sprite.coin);
                    add(coin);
                }
            }
        }
    ...
    public Entity getEntity(BoundingBox boundingBox) {
        for (int i = 0; i < level_objects.size(); i++) {
            if(((Entity)level_objects.get(i)).getBoundingBox().collided(boundingBox.getCollisionBox())){
